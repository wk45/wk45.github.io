...
::: {layout-ncol=2}
![1](slide_images/PPD_lam_selection1.png)

![2](slide_images/ppd_lam_selection2.png)
:::
...
## **Brief Introductory**{.smaller}

A set of partial elastic means, $\{\hat{g}_{\lambda_l}\}$, is a sequence of functions parameterized by $\lambda$.

It is meaningful to study the evolution of $\hat{g}_\lambda$ in a lower-dimensional space, called the **trajectory**.

The trajectory can be constructed in the pricipal component subspace of (PCA) of $\{\hat{g}_{\lambda_l}\}$.

## Brief Introductory {.smaller}
The raw trajectory is not smooth due to local jitter in the alignment.

This requires a smooth approximation of the trajectory, called the **principal curve**.

A principal curve, $S$, is parameterized as $\mathbf{s}(\xi)$, where $\xi \in [0,1]$ is the arc length normalized by the total length of $S$.

By matching the trajectory points to $S$, we can obtain a smooth trajectory that preserves the order of $\lambda$ progression. Then we can approximately peek the density of $\hat{g}_\lambda$ on $S$, and identify the **stable regions.**

<!-- A stable region is a high-density region in the principal component space, indicating stabilized alignment behavior.

We choose an optimal $\lambda$ where the density of the trajectory is maximized in the early stage of the trajectory.

The correlation matrix $\mathbf{R}$ refines the upper bound for $\lambda$ for the early stage of the trajectory. -->

## **Overview Illustration** {.smaller auto-animate=true}
![](slide_images/pc_trajectory.png)
...
![](slide_images/corr.png){width=50%}

![](slide_images/traj.png){width=50%}
